...
import * as Speech from 'expo-speech'; // Import the expo-speech package
...
const App = () => {
  ...
  const [shouldSpeak, setShouldSpeak] = useState<boolean>(false); // Flag to control speech synthesis
      ...
      try {
        ...
        if (response.body.features.length > 0) {
          ...
          if (distanceToDestination <= maxDistance) {
            setShouldSpeak(true); // Set the flag to true to enable speech synthesis
            setDestination([longitude, latitude]);

            Speech.speak(`Setting navigation goal to: ${transcript}`); // Speak the recognized text
            
            Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success);
    
          } else {
            console.log(`${transcript} is too far away. Try again.`);
            Speech.speak(`${transcript} is too far away. Try again.`); // Speak the recognized text
            Vibration.vibrate([0, 200, 200, 200, 200, 200]); // Pattern: wait 0ms, vibrate 200ms, wait 200ms, vibrate 200ms, wait 200ms, vibrate 200ms
          }
        } else {
          console.log(`No valid navigation path found for ${transcript}. Try again.`);
          Speech.speak(`No valid navigation path found for ${transcript}. Try again.`)
          Vibration.vibrate([0, 200, 200, 200, 200, 200]); // Pattern: wait 0ms, vibrate 200ms, wait 200ms, vibrate 200ms, wait 200ms, vibrate 200ms
        }
      } catch (error) {
  ...
  const calculateDirections = async () => {
    if (currentLocation && destination) {
      ...
      if (response.body && response.body.routes.length > 0) {
        ...
        if (shouldSpeak) {
          const walkingTimeMinutes = Math.round(response.body.routes[0].duration / 60);
          Speech.speak(`Estimated walking time: ${walkingTimeMinutes} minutes.`);
          setShouldSpeak(false); // Reset the flag
        }
  ...
  const calculateNearestPointDistanceAndSide = (location: Location.LocationObject, route: any) => {
    ...
    const [x1, y1] = route.coordinates[0];
    const [x2, y2] = route.coordinates[1];
    const [px, py] = [location.coords.longitude, location.coords.latitude];
    const crossProduct = (x2 - x1) * (py - y1) - (y2 - y1) * (px - x1);

    const side = crossProduct > 0 ? 'left' : 'right';

    return { distanceToNearestPoint, side };
  };

  const mapDistanceToValue = (distance: number) => {
    if (distance <= 3) return null;
    if (distance >= 25) return 9;
    return Math.floor(((distance - 3) / (25 - 3)) * 9);
  };
  ...
  useEffect(() => {
    ...
    if (currentLocation && route) {
      const { distanceToNearestPoint, side } = calculateNearestPointDistanceAndSide(currentLocation, route);
      console.log("Distance to route:", distanceToNearestPoint);
      console.log("Side of the path:", side);

      if (distanceToNearestPoint > 35) {
        console.log("Calculating new route");
        Vibration.vibrate(300);
        calculateDirections();
      } else {
        let value = mapDistanceToValue(distanceToNearestPoint);
        if (value !== null) {
          if (side == 'left') {
            value += 10;
            writeToDevice(value.toString(), connectedDevice);
          } else {  // side === 'right'
            value += 20;
            writeToDevice(value.toString(), connectedDevice);
          }
  ...
  const handleMapPress = async (event: OnPressEvent) => {
    ...
    if (currentLocation) {
      ...
      if (distanceToDestination <= maxDistance) {
        ...
        Speech.speak("Destination is too far.");
        Vibration.vibrate([0, 200, 200, 200, 200, 200]); // Pattern: wait 0ms, vibrate 200ms, wait 200ms, vibrate 200ms, wait 200ms, vibrate 200ms
      }
   }
  };
